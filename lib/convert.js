define([
    ...
    function init(callbackIfPreload) {
        ...
        if (window.webkitStorageInfo) {
            window.webkitStorageInfo.requestQuota(PERSISTENT, FILE_QUOTA, function(grantedBytes) {
                window.requestFileSystem(PERSISTENT, grantedBytes, onInitFs, fileApiErrorHandler);
            }, function(e) {
                console.log('FS Error', e);
            });
        } else {
            alert('Hey! Your browser is not having file system API support. This might cause problems with caching. Try download the latest chrome!');
        }
    ...
    function startParsing(file) {
        ...
        parser.parse(file, convertViewModel.language()).then(function(userData) {
            var userName = utils.getUserNameFromZipFileName(file.name);

            // deferred.resolve(userData);

            loadDataToUser(userData.userFilteredData, userData.userActivity);

            try {
                writeToFile(userName + '.filtered.json', JSON.stringify(userData.userFilteredData));
