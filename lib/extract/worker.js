        ...
        function parseAndAnalyze(file, language) {
            ...
            zipUtils.getMessagesRawFromJSZip(file).then(
                    function(rawData) {
                        ...
                        var processedData = analyze.analyze(extractedData);

                        postMessage({
                            command: 'message',
                            message: 'Found main usernames: ' + processedData.parsingMetaData.mainUserNames + '. Converting to monthly activity...'
                        });

                        userActivity = convert.convert(processedData);
